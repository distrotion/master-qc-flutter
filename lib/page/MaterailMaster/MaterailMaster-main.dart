...
class MatMasterTableINPUT extends StatelessWidget {
  const MatMasterTableINPUT({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MatMasterTableBUFFER();
  }
}

class MatMasterTableBUFFER extends StatelessWidget {
  const MatMasterTableBUFFER({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MatMasterTable();
  }
}

class MatMasterTable extends StatefulWidget {
  MatMasterTable({Key? key, this.data}) : super(key: key);
  List<dataset>? data;

  @override
  _MatMasterTableState createState() => _MatMasterTableState();
}

class _MatMasterTableState extends State<MatMasterTable> {
  ...
  _MyData(this.context, this.input) {
    ...
    for (int i = 0; i < _data.length; i++) {
      if (_data[i].f01.toLowerCase().contains(_searchResult) ||
          ...
          _data[i].f07.toLowerCase().contains(_searchResult) ||
          _data[i].f08.toLowerCase().contains(_searchResult)) {
